...
async def create_text_backup():
    """Создает простой текстовый бэкап структуры базы"""
    try:
        print("📝 Создаем текстовый бэкап...")
        
        backup_content = []
        backup_content.append(f"=== БЭКАП БАЗЫ ДАННЫХ ===")
        backup_content.append(f"Дата: {datetime.datetime.now(TZ)}")
        backup_content.append(f"База: {DB_NAME}")
        ...
        backup_content.append(f"📊 Всего таблиц: {len(tables)}")
        backup_content.append("")
        
        # Для каждой таблицы получаем количество записей
        for i, table in enumerate(tables, 1):
            print(f"📋 Обрабатываем таблицу {i}/{len(tables)}: {table}")
            ...
            backup_content.append(f"--- ТАБЛИЦА: {table} ---")
            backup_content.append(f"Записей: {count[0]}")
            backup_content.append(f"Колонки: {len(columns)}")
            
            # Информация о колонках
            backup_content.append("Структура:")
            for col in columns[:5]:  # Первые 5 колонок
                backup_content.append(f"  - {col[0]} ({col[1]})")
            if len(columns) > 5:
                backup_content.append(f"  ... и еще {len(columns) - 5} колонок")
            
            # Примеры данных
            if sample_rows:
                backup_content.append("Примеры данных:")
                for j, row in enumerate(sample_rows[:2]):  # Первые 2 строки
                    row_preview = " | ".join(str(cell)[:30] + "..." if len(str(cell)) > 30 else str(cell) for cell in row[:3])  # Первые 3 ячейки
                    backup_content.append(f"  {j+1}. {row_preview}")
            
            backup_content.append("")
        
        # Создаем временный файл
        timestamp = datetime.datetime.now(TZ).strftime('%Y%m%d_%H%M%S')
        backup_filename = f"backup_info_{timestamp}.txt"
        
        with tempfile.NamedTemporaryFile(mode='w', suffix='.txt', delete=False, encoding='utf-8') as f:
            f.write("\n".join(backup_content))
            backup_path = f.name
        
        print(f"✅ Текстовый бэкап создан: {backup_path}")
        return backup_path
        
    except Exception as e:
        print(f"❌ Ошибка создания текстового бэкапа: {e}")
        import traceback
        print(f"🔍 Детали: {traceback.format_exc()}")
        return None

async def create_sql_dump_simple():
    """Создает SQL дамп только структуры (без данных)"""
    try:
        print("🗃️ Создаем SQL дамп структуры...")
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with open(backup_path, 'w', encoding='utf-8') as f:
                f.write("-- SQL Dump - Structure Only\n")
                f.write(f"-- Generated by Arcady Bot at {datetime.datetime.now(TZ)}\n")
                f.write(f"-- Database: {DB_NAME}\n\n")
                
                f.write("SET FOREIGN_KEY_CHECKS=0;\n\n")
                
                for table in tables:
                    print(f"🔄 Экспортируем структуру: {table}")
            ...
            if os.path.exists(backup_path) and os.path.getsize(backup_path) > 0:
                file_size = os.path.getsize(backup_path)
                print(f"✅ SQL дамп создан: {backup_path} ({file_size} bytes)")
                return backup_path
            else:
                print("❌ SQL дамп не создан")
                return None
...
@dp.message(Command("backup_text"))
async def cmd_backup_text(message: types.Message):
    """Отправляет текстовый бэкап информации о базе"""
    if message.from_user.id not in ALLOWED_USERS:
        await message.answer("❌ У вас нет прав для этой команды")
        return
    
    await message.answer("📝 Создаю текстовый бэкап...")
    
    backup_path = await create_text_backup()
    if not backup_path:
        await message.answer("❌ Не удалось создать текстовый бэкап")
        return
    ...
    try:
        with open(backup_path, 'rb') as backup_file:
            await message.answer_document(
                backup_file,
                caption=f"📋 Информация о базе данных\n"
                       f"📅 {datetime.datetime.now(TZ).strftime('%d.%m.%Y %H:%M')}"
            )
        
        await message.answer("✅ Текстовый бэкап отправлен!")
...
@dp.message(Command("backup_structure"))
async def cmd_backup_structure(message: types.Message):
    """Отправляет SQL дамп структуры базы"""
    if message.from_user.id not in ALLOWED_USERS:
        await message.answer("❌ У вас нет прав для этой команды")
        return
    
    await message.answer("🗃️ Создаю SQL дамп структуры...")
    
    backup_path = await create_sql_dump_simple()
    if not backup_path:
        await message.answer("❌ Не удалось создать SQL дамп")
        return
    ...
    try:
        with open(backup_path, 'rb') as backup_file:
            await message.answer_document(
                backup_file,
                caption=f"🗃️ SQL дамп структуры БД\n"
                       f"📅 {datetime.datetime.now(TZ).strftime('%d.%m.%Y %H:%M')}"
            )
        
        await message.answer("✅ SQL дамп отправлен!")
...
@dp.message(Command("quick_backup"))
async def cmd_quick_backup(message: types.Message):
    """Быстрый бэкап - пробует разные методы"""
    if message.from_user.id not in ALLOWED_USERS:
        await message.answer("❌ У вас нет прав для этой команды")
        return
    
    await message.answer("⚡ Пробую разные методы бэкапа...")
    
    # Пробуем текстовый бэкап
    backup_path = await create_text_backup()
    if backup_path:
        try:
            with open(backup_path, 'rb') as backup_file:
                await message.answer_document(
                    backup_file,
                    caption=f"📋 Быстрый бэкап - информация\n"
                           f"📅 {datetime.datetime.now(TZ).strftime('%d.%m.%Y %H:%M')}"
                )
            await message.answer("✅ Текстовый бэкап отправлен!")
            if os.path.exists(backup_path):
            ...
            print(f"Ошибка отправки текстового бэкапа: {e}")
    
    # Если текстовый не сработал, пробуем SQL структуру
    await message.answer("🔄 Пробую SQL дамп структуры...")
    backup_path = await create_sql_dump_simple()
    if backup_path:
        try:
            with open(backup_path, 'rb') as backup_file:
                await message.answer_document(
                    backup_file,
                    caption=f"🗃️ Быстрый бэкап - структура\n"
                           f"📅 {datetime.datetime.now(TZ).strftime('%d.%m.%Y %H:%M')}"
                )
            await message.answer("✅ SQL дамп отправлен!")
            if os.path.exists(backup_path):
            ...
            print(f"Ошибка отправки SQL дампа: {e}")
    
    # Если ничего не сработало
    await message.answer("❌ Не удалось создать бэкап ни одним методом")

# Обновляем ежедневный бэкап
async def send_daily_backup():
    """Ежедневная отправка бэкапа админам"""
    try:
        print("🔄 Запуск ежедневной отправки бэкапа...")
        
        # Используем текстовый бэкап (самый надежный)
        backup_path = await create_text_backup()
        if not backup_path:
